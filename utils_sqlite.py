    ...
    if os.path.isfile(db_path):
        ...
        try:
            ...
            if dict_key_types_sel:
                ...
                sql = r'SELECT %s FROM %s' % (key_f_t, table_name)
...
def create_table(db_path, table_name, dict_keys_type, primary_key=None,
                 with_time_stamp=True, key_time_stamp=KeyInsertTimeStamp):
    ...
    if table_name not in table_names:
        ...
        dict_sql_keys = {k: "%s %s" % (k, v) for k, v in dict_keys_type.items()}
        if primary_key in dict_sql_keys.keys():
            dict_sql_keys[primary_key] = dict_sql_keys[primary_key] + r' PRIMARY KEY NOT NULL'

        if with_time_stamp:
            sql_time_stamp = '%s TimeStamp NOT NULL DEFAULT (datetime(\'now\',\'localtime\'))' % key_time_stamp
            dict_sql_keys[key_time_stamp] = sql_time_stamp

        sql_main = "CREATE TABLE %s(%s);" % (table_name, ','.join(dict_sql_keys.values()))
        with my_sqlite3(db_path) as cursor_temp:
            cursor_temp.execute(sql_main)
...
def delete_list(db_path, table_name, ref_key, list_value_ref):
    ...
    if table_name in table_names:
        ...
        sql = r'DELETE FROM {} WHERE {}={}'.format(table_name, ref_key, value_ref_f_t)
        with my_sqlite3(db_path) as cursor_temp:
            cursor_temp.executemany(sql, list_list_value_ref)


def insert_from_list_to_db(db_path, table_name, list_keys=None, list_data=None, primary_key=None,
                           with_time_stamp=True, key_time_stamp=KeyInsertTimeStamp,
    ...
    if list_keys and list_data:
        ...
        dict_keys_type = {list_keys[h]: dict_keys_type_set[list_keys[h]] if list_keys[
                                                                                h] in dict_keys_type_set.keys() else get_type_str_list(
            [x[h] for x in list_data]) for h in range(len(list_keys))}
        ...
        sql = r'INSERT OR IGNORE INTO "{}" {} VALUES{}'.format(table_name, key_f_t, data_f_t)
        with my_sqlite3(db_path) as cursor_temp:
            cursor_temp.executemany(sql, list_data)
    ...
    try:
        if list_keys_update and list_data_update:
            ...
            if table_name in table_names:
                ...
                dict_keys_type = {
                    list_keys_update_t[h]: dict_keys_type_set[list_keys_update_t[h]] if list_keys_update_t[
                                                                                            h] in dict_keys_type_set.keys() else get_type_str_list(
                        [x[h] for x in list_data_update_t]) for h in range(len(list_keys_update_t))}
                ...
                sql = r'UPDATE {} SET {} WHERE {}{}'.format(table_name, key_and_data_f_t, key_and_data_ref_t, str_where_t)
